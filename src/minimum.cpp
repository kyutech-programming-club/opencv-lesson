...
int main(int argc, char** argv)
{
  ...
  image_transport::Publisher image_pub {pit.advertise("image_raw", 1)};//image_rawトピックへの吐き出しを指定して, データサイズを"1"に指定
  image_transport::Subscriber image_sub {it.subscribe<sensor_msgs::Image>("input_image", 1, [&image_pub](const sensor_msgs::ImageConstPtr& msg){
    try {
